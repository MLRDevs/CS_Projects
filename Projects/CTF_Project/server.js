...
// check if the users image is an image of a flag of the active contest
async function CheckImage(image, email) {

	// if active image is ubuntu don't bother checking everything
	if (image.ActiveFlag === 'ubuntu') {
		let data = { ActiveFlag: 'ubuntu' };
		const username = getUsername(email);
		return await StartContainer(data,username, email);
	}

	let data = { ActiveFlag: null };
	const username = getUsername(email);

	// get the active contest for user
	return getActiveContest(email).then(async (contest) => {
		
		// no contest so start ubuntu
		if (contest === null || contest === undefined) {
			console.log('starting with ubuntu');
			data.ActiveFlag = 'ubuntu';
			UpdateImage(email,data.ActiveFlag);
			return await StartContainer(data, username, email);
		}
		else {

			// get all the flags from contest
			return getFlagFromContestID(contest).then(async (flags) => {
				
				if (flags.length > 0) {
					for (var i=0; i < flags.length; i++) {

						// if the image exists then start that one
						if (flags[i].Image === image.ActiveFlag) {
							data.ActiveFlag = image.ActiveFlag;
							console.log('a flag has the active image: ',image);
							return await StartContainer(data, username, email);
						}
					}

					// if the image doesn't exist then start a container with one of the flags
					data.ActiveFlag = flags[0].Image;
					console.log('starting container with different flag image: ',flags[0].Image)
					UpdateImage(email,data.ActiveFlag);
					return await StartContainer(data, username,email);
				}
				else {
...
var con = mysql.createConnection({
	// this data was removed due to having personal information
})
...
app.post('/AddContest', (req,res) => {
	const { Name, IsActive, email } = req.body;
	return getAdminID(email).then((Admin) => {
		const query = 'INSERT INTO CONTESTS (Name, IsActive, AdminID) VALUES (?,0,?)';
		con.query(query, [Name, Admin.AdminID], (err) => {
		if (err) {
	...
	return getAdminID(email).then((Admin) => {
		...
		con.query(query, [Admin.AdminID], (err,rows) => {
			...
			if (rows.length === 0){
				...
				return res.status(400).json({ error: 'CONTEST ALREADY SET ACTIVE' });
			}
		});
...
app.post('/DeleteFlagsFromContest', (req,res) => {
	...
	con.query(query, [contest], (err) => {
		if (err) {
			console.error(err.message);
			return res.status(500).json({ error: 'DELETING ALL FLAGS' });
		}
...
app.post('/DeleteImageReplaceFlag', (req,res) => {
	const { flag, image } = req.body;
	let query = 'DELETE FROM Images WHERE Name = ?';
	con.query(query, [image], (err) => {
		if (err) {
			console.error(err.message);
			return res.status(500).json({ error: 'DELETING IMAGE AND REPLACING FLAG '});
		}
		if (this.changes === 0) return res.status(404).json({ error: 'NO IMAGE FOUND' });
		else {
			query = 'UPDATE Flags SET Image = ? WHERE FlagID = ?';
			con.query(query, ['ubuntu',flag], (err) => {
				if (err) {
					console.error(err.message);
					return res.status(500).json({ error: 'REPLACING FLAG' });
				}
				return res.status(200).json({ success: true });
			});
		}
...
// get the current active contest name
async function getActiveContest(email) {
	return new Promise((resolve, reject) => {
		return AdminorUser(email).then((AoU) => {
			if (AoU === 'Admins') {
				return getAdminID(email).then((Admin) => {
					const query = 'SELECT ContestID FROM Contests WHERE IsActive = 1 AND AdminID = ?';
					con.query(query, [Admin.AdminID], (err,rows) => {
						if (err) {
							console.error(err.message);
							reject(err);
...
async function getFlagFromContestID(contest) {
	return new Promise((resolve, reject) => {
		const query = 'SELECT Image FROM Flags WHERE ContestID = ?';
		con.query(query, [contest.ContestID], (err,rows) => {
			if (err) {
				console.error(err.message);
				reject(err);
			}
...
// update the users active flag image
async function UpdateImage(email,image) {
	AdminorUser(email).then((AoU) => {
		const query = `UPDATE ${AoU} SET ActiveFlag = ? WHERE Email = ?`;
		con.query(query, [image, email], (err) => {
			if (err) {
				console.error(err.message);
			}
		})
...
function CreateImage(root, imgname, email, files) {
	...
	getAdminID(email).then(async (Admin) => {
		...
		// delete old dockerfile if there is one
		const files = fs.readdirSync(AdminDir);
		if (files[0] !== undefined) {
			if (path.extname(files[0]) == '.dockerfile') fs.unlinkSync(AdminDir + '/' + files[0]);
		}
		...
		async function getContents(node, filepath) {
			...
			if (node.directory === true) {
				...
				// if the root path already exists then delete it
				if (fs.existsSync(filepath + '/' + node.name)) DeleteOldRoot();
		...
		async function DeleteOldRoot() {
			
			// delete root folder and dockerfile after creating docker image
			const rootpath = path.join(AdminDir,'Root');
			fs.rmSync(rootpath, {recursive: true, force: true }, err => {
				if (err) {
					console.error(err.message);
					return;
				}
